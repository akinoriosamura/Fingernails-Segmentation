...
class fingernailseg:
    ...
    def __init__(self, dataset):
        if len(dataset):
            ...
            tar.extractall()
            tar.close()
        # dir_name = os.path.splitext(os.path.basename(dataset))[0]
        dir_name = dataset[:-7]
        mask_files = os.listdir(os.path.join(dir_name, 'mask'))
        raw_files = os.listdir(os.path.join(dir_name, 'raw'))
        ...
        y = []
        for f in files:
            mask = Image.open(os.path.join(dir_name, 'mask/'+f)).convert('RGB')
            ...
            if not mask.sum():
                continue
            y.append(mask)
            raw = Image.open(os.path.join(dir_name, 'raw/'+f)).convert('RGB')
