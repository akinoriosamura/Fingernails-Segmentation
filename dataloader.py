...
class DataLoader(object):
    ...
    def __init__(self, images, masks, image_size: Tuple[int],
                 channels: Tuple[int] = (3, 3), crop_percent: float = None, seed: int = None,
                 augment: bool = True, compose: bool = False, palette=None):
        """
    ...
    @tf.function
    def _map_function(self, image, mask):
        # image, mask = self._parse_data(image, mask)
        ...
        def _augmentation_func(image_f, mask_f):
            ...
            image_f, mask_f = self._normalize_data()(image_f, mask_f)
    ...
    def process(self, shuffle=False):
        ...
        # data = tf.data.Dataset.from_tensor_slices((self.images, self.masks))
        data = (self.images, self.masks)

        # Parse images and labels
        data = data.map(self._map_function, num_parallel_calls=AUTOTUNE)
        data = map(self._map_function, data)

        if shuffle:
            # Prefetch, shuffle then batch
            data = data.prefetch(AUTOTUNE).shuffle(random.randint(0, len(self.image_paths)))
        else:
            # Batch and prefetch
